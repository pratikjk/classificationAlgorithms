...
public class Instances {
    ...
    public boolean areAllSameClass() {
        for (int i = 1; i < getDataSetSize(); i++) {
            if (getInstance(i).getClassValue() != getInstance(i - 1)
                    .getClassValue()) {
                return false;
            }
        }
    ...
    public Instances getTrainingForCrossValidation(int numOfFolds,
            int foldToValidate) {
        ...
        train.add(getInstancesSubset(0,
                ((foldToValidate - 1) * instancePerFold)));
        // System.out.print(0 +" to "+((foldToValidate-1)*instancePerFold));
        setTestForCrossValidation(getInstancesSubset(
                ((foldToValidate - 1) * instancePerFold), foldToValidate
                        * instancePerFold));
        train.add(getInstancesSubset(foldToValidate * instancePerFold,
                (numOfFolds * instancePerFold)));
        // System.out.print("\t"+foldToValidate*instancePerFold
        // +" to "+numOfFolds*instancePerFold);
        train.add(getInstancesSubset(numOfFolds * instancePerFold,
                (numOfFolds * instancePerFold) + remainder));
        // System.out.println("\t"+numOfFolds*instancePerFold
        // +" to "+((numOfFolds*instancePerFold)+remainder));
        return train;
    }
