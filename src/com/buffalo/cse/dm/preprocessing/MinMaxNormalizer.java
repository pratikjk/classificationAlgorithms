...
public class MinMaxNormalizer implements Normalizer {
	...
	private void normalizeByColumn(Instances dataset, int colIndex) {
		...
		for(int i=0;i<dataset.getDataSetSize();i++){
			double currValue=dataset.getInstance(i).getAttribute(colIndex).getAttributeValue();
			if(max<currValue){
				max=currValue;
			}
			if(min>currValue){
		...
		for(int i=0;i<dataset.getDataSetSize();i++){
			...
			dataset.getInstance(i).getAttribute(colIndex).setAttributeValue(newValue);
		}
		
	}

	private double minMaxNormalization(double currValue, double min,
			double max) {
